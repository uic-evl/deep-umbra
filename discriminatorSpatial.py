...
def DiscriminatorSpatial():
    initializer = tf.random_normal_initializer(0., 0.02)

    inp = tf.keras.layers.Input(shape=[256, 256, 3], name='input_image')
    tar = tf.keras.layers.Input(shape=[256, 256, 3], name='target_image')
    lat = tf.keras.layers.Input(shape=[256, 256, 1], name='latitude')

    x = tf.keras.layers.concatenate([inp, tar, lat])  # (batch_size, 256, 256, channels*2)
    ...
    return tf.keras.Model(inputs=[inp, tar, lat], outputs=last)
